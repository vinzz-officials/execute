...
if (text.startsWith("/post ")) {
  try {
    // Ambil content Telegram
    let content = text.slice(6).trim();
    let cookie = null;

    // Ambil cookie jika ada (diakhiri | cookie)
    if (content.includes("|")) {
      const parts = content.split("|");
      content = parts[0].trim(); // sebelum |
      cookie = parts[1].trim();  // cookie
    }
    ...
    try {
      ...
      await send(chat_id, "ERROR: JSON invalid.\n" + err.toString(), BOT_TOKEN);
      return res.json({ ok: true });
    }

    // Fetch POST dengan cookie
    ...
    if (out.length > 3500) {
      const form = new FormData();
      const filename = `post_result_${Date.now()}.txt`;
      form.append("chat_id", chat_id);
      form.append("caption", "Output kepanjangan, dikirim sebagai file.");
      form.append("document", Buffer.from(out), {
        filename,
